...
class Utility
{
    ...
    /** 根据提供的键值对，创建radio组，并选中对应的值。 */
    public static function strOfRadioSimple($name,$options,$selectValue=null,$strAttrs='')
    {
        if (!isset($selectValue))
        {
            if (isset($GLOBALS['params'][$name]))
        ...
        foreach ($options as $oValue => $oName)
        {
            $s .=   '<label style="margin-right:30px;">'
                        .'<input style="margin-right:10px;" type="radio" name="'.$name.'" autocomplete="off" value="'.$oValue.'"' . (strval($selectValue)===strval($oValue)?' checked':'') .' '.$strAttrs. '>'
                        .$oName
                    .'</label>';
        }
        $s .= '</div>'."\n";
    ...
    /** 报错并返回503信息 */
    public static function exitMessage($message,$title=null)
    {
        include AXAPI_ELO_PATH .'/exit.php';
        exit;
    }
